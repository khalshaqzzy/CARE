...
- Classification system prompt bersifat code-owned immutable (saat ini `care-classification-v1.7`) dan menanamkan pertahanan prompt-injection, panduan pemilihan satu primary category paling dominan beserta batas antar kategori, rubrik severity §13.4 beserta contoh per level, kalibrasi confidence terhadap threshold fallback, dan kontrak tool call. Definition dan Examples kategori tetap structured context dinamis dari katalog database. Versi `v1.6` memperjelas enam Definition kategori bawaan; `v1.7` hanya memperkuat rubric severity agar kondisi aktif, near miss, skala, keberulangan, dan dampak konkret tidak under-classified. Category guidance, Definition, Examples, confidence, schema, dan provider settings tidak berubah pada `v1.7`. Setiap perubahan konten prompt atau structured classification context wajib menaikkan versi prompt.
...
Severity dinilai terpisah dari category berdasarkan dampak terberat yang konkret,
urgensi, skala, keberulangan, dan kondisi atau paparan yang masih berlangsung.
Ketiadaan cedera yang sudah terjadi tidak menurunkan near miss atau kondisi aktif
dengan konsekuensi serius yang kredibel. Kata risiko saja tidak cukup untuk
menaikkan level. Kekurangan fakta menurunkan confidence, bukan otomatis severity.

| Severity | Meaning                                                                                                                               | Contoh                                                                                                                                                                                           |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Low      | Usulan, informasi, apresiasi, atau ketidaknyamanan minor tanpa dampak material yang sedang terjadi                                    | Ide label/warna, variasi menu, retak kosmetik stabil, perbaikan kenyamanan kecil                                                                                                                 |
| Medium   | Masalah nyata tetapi terbatas/terkendali; tidak ada bahaya langsung atau gangguan besar dan workaround cukup                          | Sebagian lampu mati tetapi area layak, alat kecil rusak dengan backup, SOP kurang jelas, delay kecil berulang, antrean fasilitas                                                                 |
| High     | Dampak signifikan atau risiko serius kredibel yang perlu segera ditangani tetapi belum berupa emergency aktif                         | Pekerjaan berbahaya tanpa APD wajib, paparan menimbulkan gejala, kondisi unsafe berulang, layanan penting tidak tersedia bagi banyak orang, gangguan equipment/manpower dan people impact serius |
| Critical | Emergency aktif/berkembang, potensi langsung cedera berat, serious people/compliance issue, atau dampak besar yang sedang berlangsung | Api/asap/panel panas, severe near miss, unsafe machine aktif, tumpahan besar ke drainase, total line stop/customer risk segera, harassment/discrimination dengan ancaman atau retaliation        |
...
- Membaca detail tidak mengubah status. Respons wajib keterangan 1–4.000 karakter setelah trim.
...
- Sebelum assignment/proses, route tujuan tetap ditampilkan; sesudahnya PIC efektif ditampilkan. Private memakai Komite.
- Proses meminta target wajib integer 0–365 hari. Tidak ada pilihan otomatis. Pilihan cepat: Hari ini, 1, 3, 7, 14 hari.
- Hari dihitung dari tanggal server di Asia/Jakarta. Nol berarti hari ini; satu berarti besok. Deadline adalah akhir hari 23.59.59,999 WIB, disimpan UTC, ditampilkan tanggal/jam 23.59 WIB.
- Bottom sheet menampilkan pratinjau; target final mengikuti waktu server saat transaksi. Target immutable per siklus penanganan, dengan hari, actor, waktu penetapan, deadline dan penanda notifikasi keterlambatan.
...
- Label nama mengambil nama lengkap dan menampilkan maksimal 12 karakter: 11 karakter + elipsis jika terpotong. Nama lengkap tersedia pada detail peserta dan aksesibilitas.
- Header ringkas memuat konteks, status, avatar dan peran peserta. Pesan dikelompokkan per hari WIB; composer tetap terjangkau di atas dock/safe area.
- Draft teks/lampiran baru dibersihkan setelah kirim berhasil; retry memakai idempotency yang sama untuk payload sama. Pesan baru tidak memaksa scroll ketika pengguna membaca riwayat.
- Text 1–4.000 karakter; maksimum lima gambar, 10 MB/file. Pesan kosong tanpa attachment ditolak. Pesan tidak diedit/dihapus. Close dan message menggunakan lock Voice yang sama.
- Aksesibilitas, responsive layout dan legacy WebKit tetap wajib. Presence/read receipt tidak ditambahkan.
...
type VoiceStatus = 'OPEN' | 'RESPONDED' | 'IN_PROGRESS' | 'CLOSED';
...
- Voice wajib `GENERAL`, `OPEN`, dan `currentHandlerId=null`. Handover tidak
  ...
  CARE Admin, previous PIC, Private Voice, atau status `RESPONDED`,
...
filter, dashboard, Admin dan API memakai RESPONDED; Admin summary memakai monitored.
...
Dashboard responder, leadership, General Penanganan/Pelaporan dan Union Private menampilkan Average Response Time, Average Completion Time dan Performance. Response merupakan rata-rata submit → event RESPONDED pertama per Voice, termasuk assignment otomatis. Completion menghitung setiap siklus selesai: submit → closed pertama, lalu reopenedAt siklus sebelumnya → closed berikutnya. Performance memakai seluruh Rating.score, termasuk rating pemicu reopen. Reopen langsung ke In Progress tidak membuat sampel response baru.
