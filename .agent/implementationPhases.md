...
| Current implementation | Production delivery remains locally verified. QA-001–004 are resolved through set-oriented import, Sidebar contract alignment, immediate auth invalidation, and terminal import cache refresh; QA-005 is deferred and QA-006 is resolved (ADR-0013). The live OpenAI-compatible provider smoke now emits traceable diagnostics and is advisory during auto-deploy (non-blocking; recorded to deployment-state), while release readiness still requires it to pass (ADR-0015). Full local parity is green |
