...
required=(CADDY_EMAIL POSTGRES_USER POSTGRES_PASSWORD POSTGRES_DATABASE SESSION_HASH_SECRET SESSION_CSRF_SECRET AUTH_THROTTLE_SECRET CURSOR_SIGNING_SECRET METRICS_TOKEN CARE_ADMIN_USERNAME CARE_ADMIN_PASSWORD OPENAI_API_KEY OPENAI_CONFIG_ENCRYPTION_KEY OPENAI_MODEL OPENAI_BASE_URL VAPID_SUBJECT VAPID_PUBLIC_KEY VAPID_PRIVATE_KEY)
...
printf 'OPENAI_REASONING_EFFORT=%s\nOPENAI_CONFIDENCE_THRESHOLD=%s\nOPENAI_TIMEOUT_MS=%s\n' "${OPENAI_REASONING_EFFORT:-}" "${OPENAI_CONFIDENCE_THRESHOLD:-0.75}" "${OPENAI_TIMEOUT_MS:-60000}"
printf 'PUSH_ENDPOINT_HOSTS=%s\nPUSH_CANARY_ENDPOINT_HASH=%s\n' "${PUSH_ENDPOINT_HOSTS:-fcm.googleapis.com,updates.push.services.mozilla.com,web.push.apple.com,*.notify.windows.com}" "${PUSH_CANARY_ENDPOINT_HASH:-}"
