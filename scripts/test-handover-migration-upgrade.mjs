...
const database = 'care_handover_upgrade_test';
const latest = '20260902160000_manager_voice_handover';
...
INSERT INTO "Voice" (
  ...
  '91000000-0000-4000-8000-000000000001','GENERAL','KARAWANG_1','9001','Upgrade Reporter',
...
const after = JSON.parse(
  psql(
    ...
    `SELECT json_build_object(
      'voiceId',id::text,'categoryKey',"categoryKey",'categoryId',"categoryId"::text,
      'categoryName',"categoryNameSnapshot",'currentCategoryKey',"currentCategoryKey",
      'currentCategoryId',"currentCategoryId"::text,'currentCategoryName',"currentCategoryNameSnapshot",
      'owner',"routeOwnerId"::text,'status',status::text,'version',version,
      'events',(SELECT count(*) FROM "VoiceEvent"),'handovers',(SELECT count(*) FROM "VoiceHandover")
    ) FROM "Voice" WHERE id='92000000-0000-4000-8000-000000000001';`,
  ),
);
...
for (const key of [
  ...
  if (after[key] !== original[key])
    throw new Error(
      `Handover upgrade changed ${key}: ${JSON.stringify({ before: original, after })}`,
    );
if (
  after.currentCategoryKey !== original.categoryKey ||
  after.currentCategoryId !== original.categoryId ||
  after.currentCategoryName !== original.categoryName ||
  after.handovers !== 0
)
  throw new Error(`Handover upgrade backfill mismatch: ${JSON.stringify(after)}`);

process.stdout.write(`Handover migration upgrade passed: ${JSON.stringify(after)}\n`);
