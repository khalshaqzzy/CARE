...
Granite runs BF16, tensor parallelism one, static memory fraction 0.8, a 32,768-token context window, automatic reasoning/tool parsers, default CUDA graph and Radix cache behavior, and no quantization. CARE requests cap generation at 8,192 new tokens and use `temperature=1.0` with `top_p=0.95`. Provider-default Granite requests explicitly apply `enable_thinking=true` and `low_effort=false` as defense in depth.
